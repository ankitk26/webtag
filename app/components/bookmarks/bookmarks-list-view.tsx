...
export default function BookmarksListView() {
  ...
  return (
    <div className="flex items-center border border-border rounded-md">
      ...
      <Link to="/bookmarks" search={(prev) => ({ ...prev, view: "minimal" })}>
        <Button
          variant="link"
          size="icon"
          className={`h-9 w-9 rounded-none rounded-r-md ${view === "minimal" ? "bg-muted" : ""}`}
        >
          <Minimize2 className="h-4 w-4" />
          <span className="sr-only">Minimal view</span>
        </Button>
      </Link>
    </div>
  );
